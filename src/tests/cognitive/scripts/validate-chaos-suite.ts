...
import { ChaosTestReport } from '../types/chaos';
 ...
 */
function parseTestMetadata(files: string[]): Record<string, any> {
 ...
 */
function generateDiscoveryMetrics(metadata: Record<string, any>): any {
  const metrics = {
 ...
 */
function generateReport(discoveryMetrics: any, testMetadata: Record<string, any>): ChaosTestReport {
    ...
    if (fs.existsSync(resultsPath)) {
      ...
      if (results.testResults) {
        ...
        report.testRunSummary.duration = results.testResults.reduce(
          (sum: number, result: any) => sum + (result.endTime - result.startTime),
          0
        );
  ...
  for (const key in testMetadata) {
    ...
    if (test.skipCi) {
      ...
      (report.failureMap[key] as any).linkedDependencies = test.targets || [];
      (report.failureMap[key] as any).impact = 'none';
    }
  }
 ...
 * Generate human-readable summary
 */
function generateSummary(report: ChaosTestReport): string {
  ...
  if (skippedTests.length > 0) {
    for (const [key, data] of skippedTests) {
      ...
      summary += `- Linked Dependencies: ${(data as any).linkedDependencies?.join(', ') || 'None'}\n\n`;
    }
  } else {
